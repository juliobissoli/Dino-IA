...
class NewRuleBasedPlayer(KnowledgeEngine):
    ...
    @Rule(AND(Fact(timeEnter=P(lambda x: x < 11 and x > 8)), Fact(obHeight=P(lambda x: x < 60)), NOT(Fact(dinoHeight=P(lambda x: x < 330)))))
    def jump(self):
           self.retract(1)
    ...
    @Rule(AND(Fact(obg_height_next=P(lambda x: x < 60)), Fact(timeEnter_next=P(lambda x: x < 11 and x > 8)), Fact(dinoHeight=P(lambda x: x < 200))))   
    def doubleJump(self):
           self.retract(1)
           self.declare(Fact(action='K_UP'))
...
class NewRuleBasedKeyClassifier(KeyClassifier):
    def __init__(self):
        self.engine = NewRuleBasedPlayer()

    def keySelector(self, obDistance, obHeight, scSpeed, obWidth, diHeight, obDistance2, obg_height_next, obWidth2, dinoWidth, dinoDistance):   
        frontalPosDino = dinoDistance-dinoWidth

        print(frontalPosDino)

        timeEnter=((obDistance - frontalPosDino) / 2)/scSpeed
        timeLeave=((obDistance+obWidth - frontalPosDino) / 2)/scSpeed

        timeEnter_next=((obDistance2 - frontalPosDino) / 2)/scSpeed
        # timeLeave2=((obDistance2 + obWidth2 - frontalPosDino) / 2)/scSpeed
        #print([diHeight, obHeight, obWidth, timeEnter, timeLeave, timeEnter_next, timeLeave2, obg_height_next]) 
        self.engine.reset()
        ...
        self.engine.declare(Fact(obg_height_next=obg_height_next))
        
        self.engine.declare(Fact(timeEnter_next=timeEnter_next))
...
def main():
    ...
    aiPlayer = NewRuleBasedKeyClassifier()
    print (playGame())
